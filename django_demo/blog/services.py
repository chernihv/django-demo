from .helpers import get_list_fields_request
...
def unpack_and_save_all_blocks(request, post_id: int):
    text_blocks, code_blocks = get_list_fields_request(request, 'text_block', 'code_block')
    ...
    for image in request.FILES.getlist(key='image_block'):
        file_name = helpers.get_valid_name(image)
        ...
        helpers.save_file(image, file_name)
